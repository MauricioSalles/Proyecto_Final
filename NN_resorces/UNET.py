...
try:
    from .convBlock import ConvBlock2                  
except ImportError:
    try:
        from convBlock import ConvBlock2
    except ImportError:
...
class UNET(nn.Module):
    ...
    def __init__(
        ...
        for feature in channels:
            self.encoders.append(ConvBlock2(in_channels, feature))
            in_channels = feature
        ...
        for feature in reversed(channels):
            ...
            self.decoders.append(ConvBlock2(feature*2, feature))

        self.middle = ConvBlock2(channels[-1], channels[-1]*2)
