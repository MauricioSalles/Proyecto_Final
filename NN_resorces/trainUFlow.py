...
def fulltWarp(img,deltaX,deltaY):
    N, C, H, W = img.shape
...
def transform(image,deltaX,deltaY):
    image = image[:,:,deltaX:,deltaY:]
    image = torch.flip(image,dims=[1,2])
    return image

def inverse_transform(flow,deltaX,deltaY):
    flow = torch.flip(flow,dims=[2])
    flow[:,0,:,:] *= -1
    flow = flow[:,:,deltaX:,deltaY:]
    return flow
...
def train(LR, EPOCHS,RUNID, model,currentStep):    
    ...
    for epoch in range(EPOCHS):
        for data in trainset:
            ...
            deltaY = (step+5)%20
            (F1,F2,F3) = data
            F1augmented = transform(F1,deltaX,deltaY).to(device)
            F3augmented = transform(F3,deltaX,deltaY).to(device)
            with torch.no_grad():
                FlowsTeacher = model(F1.to(device), F3.to(device),iter)
            ...
            for id,flow in enumerate(FlowsStudent):
                warpCoords = coords + flow
                warp = warping(warpCoords,F1augmented)
                loss = uLoss.unsupLoss(F1augmented, F3augmented,warp,flow,
                                       backFlowsStudent[id],inverse_transform(FlowsTeacher[id],deltaX,deltaY),
                                       inverse_transform(backFlowsTeacher[id],deltaX,deltaY))
